...
import { FiArrowLeft, FiEdit2, FiAlertTriangle, FiX } from 'react-icons/fi';
            ...
            {children.map(child => (
              <div key={child.id} className="child-card">
                ...
                <div className="child-card-info">
                  ...
                  <p className="child-card-details">Age: {child.age} · {child.gender}</p>
                  <div className={`child-medical ${child.hasAlert ? 'alert' : 'none'}`}>
                    {child.hasAlert
                      ? <FiAlertTriangle size={13} />
                      : <MdOutlineLocalHospital size={13} />
                    }
                    <span>Medical: <span className="medical-value">{child.medical}</span></span>
                  </div>
